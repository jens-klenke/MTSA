...
To estimate moments from simulated data one can simply uses the command `cov()` respectively `cor()` , but we do it by 'hand' to get a thoroughly understanding and see the relation to the Yule-Walker equation. 

First we need to demeaning the multivariate time series. Therefore, we calculate the column means. 

```{r}
mu <- colMeans(z)
```

To demeaning the series we have now two coding options to proceed, one would be to iterating over the rows and subtracting the mean vector every time. 
...
To compute $\widehat{\rho}_0$ we just need to standardizing $\widehat{\Gamma}_0$. 
