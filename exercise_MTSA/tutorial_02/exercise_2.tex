...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Gamma0.hat <-}\StringTok{ }\KeywordTok{t}\NormalTok{(z.demeaned) }\OperatorTok{%*%}\StringTok{ }\NormalTok{z.demeaned }\OperatorTok{/}\StringTok{ }\NormalTok{(N}\DecValTok{-1}\NormalTok{) }\CommentTok{# sample covariance with  }
\CommentTok{# correction for degrees of freedom (we've demeaned the series!)}
\end{Highlighting}
\end{Shaded}

Please also note that now the first entry is transposed!
(\texttt{z.demeaned} is a data matrix and not a random vector anymore!)
...
\CommentTok{# just another way to program it, }
\CommentTok{# maybe it is more visible what's inside D.inv %*% D.inv }
