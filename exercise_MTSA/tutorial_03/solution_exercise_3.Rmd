...
Both Eigenvalues are inside the unit circle therefore the process is stationary. 
...
$\Rightarrow$ Now one can apply the formulas for a VAR(1) to check stationarity. 
...
$\Rightarrow$ column 'i' $\cdot z$ + column 'i - 1' $\forall i \in \left\{ p, \ldots, 2\right\}$ yields a traingluar matrix
...
Alternativetly, we could also use the VAR(1) approach and compute the eigenvalues.
...
All `r sum(abs(var1.eigen$values) < 1)` eigenvalues lie inside the unit circle and we get the same results.
