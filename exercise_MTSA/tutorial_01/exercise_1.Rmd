...
If we take a look at the description of the _tenstock_ dataset^[To access help-file: `?tenstock()`], we can see that it contains `r ncol(tenstocks)` variables and that we have `r nrow(tenstocks)` monthly simple returns from January 2001 to December 2011 for each of the `r ncol(tenstocks) -1` companies (one variable is the time vector).   

\begin{itemize}
  \item[a)] $\quad$
\end {itemize}

First, we will have a look at the simple returns for  _JP-Morgan Chase & Co. (JPM)_. 
...
Next, we will have a look at time series for _Morgan Stanley (MS)_.
\FloatBarrier
...
The results for the _MS_ time series are similar to those of the _JPM_.
...
Only the first lag is _relatively_ close to significant to a 5 percent significance level
...
But keep in mind that the _Ljung-Box_ test does not take $\rho_0$ into consideration. Lastly, we will plot the times series with the command `plot.ts()`.  
